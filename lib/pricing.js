...
const MAX_DIM_MM = SHEET_WIDTH_MM - SIDE_MARGIN_MM * 2; // 710mm — max across-roll (width) dimension
const MAX_LENGTH_MM = 1300;   // max along-roll (height/length) dimension for two-dimension shapes
...
//
// Width is the across-roll dimension, physically capped at the 710mm roll
// width (MAX_DIM_MM). Height is the along-roll dimension, which can run much
// longer (up to MAX_LENGTH_MM) since the roll feeds out continuously.
// Single-dimension shapes (square/circle) use one measurement for both, so
// they stay capped at the smaller 710mm limit.
function priceOrder({ shape, widthMM, heightMM, qty }) {
  const singleDim = shape === 'square' || shape === 'circle';
  const q = clamp(Math.round(Number(qty) || 1), 1, 100000);

  const w = clamp(Number(widthMM) || 0, 1, MAX_DIM_MM);
  let h;
  if (singleDim) {
    h = w; // single-dimension shapes
  } else {
    h = clamp(Number(heightMM) || 0, 1, MAX_LENGTH_MM);
  }
...
module.exports = { priceOrder, MAX_DIM_MM, MAX_LENGTH_MM, VALID_SHAPES };
